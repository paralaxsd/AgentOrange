...
partial class AgentSkills
{
    ...
    public async Task<string> ExecuteSafeCSharp(string code)
    {
        ...
        try
        {
            ...
            if (!result.Success)
            {
                var errors = string.Join(Environment.NewLine, result.Diagnostics
                    ...
                    .Select(d => d.ToString()));
                return $"[Compiler Error]{Environment.NewLine}{errors}";
            }
            ...
            if (entry == null) return "[Runtime Error]: Main method nicht gefunden";
